        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-[2rem] p-8 border-4 border-slate-200">
                ...
                <div class="flex justify-between items-center mb-10">
                    <div>
                        <h2 class="text-3xl font-black text-slate-800 uppercase tracking-tighter">Monitoring Siswa 📊
                        </h2>
                        <p class="text-slate-500 font-medium">Pantau progres belajar bahasa isyarat siswa secara
                            real-time.</p>
                    </div>

                    <div class="flex items-center gap-4">
                        <div
                            class="bg-emerald-100 text-emerald-700 px-6 py-2 rounded-full font-bold text-sm border-2 border-emerald-200">
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            @csrf
                            <button type="submit"
                                class="bg-red-50 text-red-600 px-6 py-2 rounded-full font-bold text-sm border-2 border-red-200 hover:bg-red-600 hover:text-white transition-all flex items-center gap-2">
                                Keluar Akun
                            </button>
                        </form>
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="text-slate-400 text-xs uppercase tracking-widest border-b-2 border-slate-100">
                                ...
                                <th class="py-4 px-4 text-center bg-indigo-50 text-indigo-600 rounded-t-xl">Evaluasi
                                </th>
                            </tr>
                        </thead>
                                        ...
                                        <td class="py-5 px-4 text-center {{ $i == 7 ? 'bg-indigo-50/30' : '' }}">
                                            ...
                                            @if($prog && $prog->is_completed)
                                                <div class="flex flex-col items-center">
                                                    ...
                                                    @if($prog->score > 0)
                                                        <span
                                                            class="text-[10px] font-black {{ $i == 7 ? 'text-indigo-600' : 'text-slate-500' }}">
                                                            Nilai: {{ $prog->score }}
                                                        </span>
                                                    @endif
                                                </div>
