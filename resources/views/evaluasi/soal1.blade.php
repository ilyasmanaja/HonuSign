    ...
    <div class="max-w-4xl w-full px-4 py-8 flex flex-col items-center font-fredoka text-black">
        ...
        <div
            ...
            <div class="text-center mb-8">
                <span
                    class="inline-block px-4 py-1.5 bg-[#BEE9E8] brutal-border brutal-shadow-sm rounded-xl text-xs font-black uppercase tracking-wider mb-3">Literasi</span>
                <h2 class="text-2xl md:text-3xl font-black text-black leading-snug">
                    Pilihlah gambar yang menunjukkan aktivitas membersihkan kelas!
                </h2>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-3xl mx-auto">
                ...
                <button onclick="selectChoice('B')" id="card-B"
                    class="group relative overflow-hidden aspect-[4/3] brutal-border brutal-shadow rounded-[2rem] hover:-translate-y-1 transition-all duration-300 bg-white cursor-pointer select-none">
                    @php
                        $imageName = 'ina_sapu_halaman.png';
                        $imagePath = 'images/evaluasi/' . $imageName;
                        $fileExists = file_exists(public_path($imagePath));
                    @endphp
                    @if ($fileExists)
                        <img src="{{ asset($imagePath) }}" alt="Ina menyapu halaman rumah"
                            class="w-full h-full object-cover group-hover:scale-105 transition-all duration-300">
                    @else
                        <div
                ...
                <button onclick="selectChoice('C')" id="card-C"
                    class="group relative overflow-hidden aspect-[4/3] brutal-border brutal-shadow rounded-[2rem] hover:-translate-y-1 transition-all duration-300 bg-white cursor-pointer select-none">
                    @php
                        $imageName = 'ririn_cuci_piring.png';
                        $imagePath = 'images/evaluasi/' . $imageName;
                        $fileExists = file_exists(public_path($imagePath));
                    @endphp
                    @if ($fileExists)
                        <img src="{{ asset($imagePath) }}" alt="Ririn mencuci piring"
                            class="w-full h-full object-cover group-hover:scale-105 transition-all duration-300">
                    @else
                        <div
                ...
                <button onclick="selectChoice('D')" id="card-D"
                    class="group relative overflow-hidden aspect-[4/3] brutal-border brutal-shadow rounded-[2rem] hover:-translate-y-1 transition-all duration-300 bg-white cursor-pointer select-none">
                    @php
                        $imageName = 'bayu_selokan.png';
                        $imagePath = 'images/evaluasi/' . $imageName;
                        $fileExists = file_exists(public_path($imagePath));
                    @endphp
                    @if ($fileExists)
                        <img src="{{ asset($imagePath) }}" alt="Bayu membersihkan selokan"
                            class="w-full h-full object-cover group-hover:scale-105 transition-all duration-300">
                    @else
                        <div
        ...
        function updateVisuals() {
            options.forEach(opt => {
                ...
                if (opt === selectedChoice) {
                    // Apply selection border and background to #FFF5B8
