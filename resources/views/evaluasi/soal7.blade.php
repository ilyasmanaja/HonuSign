<x-student-layout>
    ...
    <div class="max-w-4xl w-full px-4 py-8 flex flex-col items-center font-fredoka text-black">
        ...
        <div
            ...
            <div class="text-center mb-8">
                <span
                    class="inline-block px-4 py-1.5 bg-[#FFD1E3] brutal-border brutal-shadow-sm rounded-xl text-xs font-black uppercase tracking-wider mb-3">Spasial</span>
                <h2 class="text-2xl md:text-3xl font-black text-black leading-snug">
                    Apa yang sedang dilakukan oleh anak-anak tersebut?
                </h2>
                <p class="text-sm text-slate-500 mt-2 font-bold uppercase tracking-widest">💡 Pilih salah satu jawaban
                    di bawah!</p>
            </div>

            <!-- Image display card -->
            <div
                class="w-full max-w-xl mx-auto mb-8 bg-white brutal-border brutal-shadow rounded-3xl overflow-hidden aspect-[16/9] relative">
                @php
                    $imageName = 'tari_zapin_riau.png';
                    $imagePath = 'images/evaluasi/' . $imageName;
                    $fileExists = file_exists(public_path($imagePath));
                @endphp
                @if ($fileExists)
                    <img src="{{ asset($imagePath) }}" alt="Tari Zapin Riau" class="w-full h-full object-cover">
                @else
                    <div
                        class="w-full h-full flex flex-col items-center justify-center p-6 bg-[#FFD1E3] text-center text-black">
            ...
            <div class="flex flex-col sm:flex-row gap-4 max-w-2xl mx-auto">
                <button onclick="selectOption('A')" id="btn-option-A"
                    class="flex-1 p-5 bg-white brutal-border brutal-shadow-sm brutal-hover rounded-2xl font-black text-black text-xl cursor-pointer flex items-center justify-center gap-4">
                    <span
                        class="w-9 h-9 bg-[#BEE9E8] rounded-xl flex items-center justify-center font-black text-sm border-2 border-black flex-shrink-0">A</span>
                    Menari
                </button>
                <button onclick="selectOption('B')" id="btn-option-B"
                    class="flex-1 p-5 bg-white brutal-border brutal-shadow-sm brutal-hover rounded-2xl font-black text-black text-xl cursor-pointer flex items-center justify-center gap-4">
                    <span
                        class="w-9 h-9 bg-[#FFD1E3] rounded-xl flex items-center justify-center font-black text-sm border-2 border-black flex-shrink-0">B</span>
                    Belajar
                </button>
                <button onclick="selectOption('C')" id="btn-option-C"
                    class="flex-1 p-5 bg-white brutal-border brutal-shadow-sm brutal-hover rounded-2xl font-black text-black text-xl cursor-pointer flex items-center justify-center gap-4">
                    <span
                        class="w-9 h-9 bg-[#E0BBE4] rounded-xl flex items-center justify-center font-black text-sm border-2 border-black flex-shrink-0">C</span>
                    Tidur
                </button>
            </div>
        </div>
    ...
    <script>
        ...
        function updateVisuals() {
            options.forEach(opt => {
                const btn = document.getElementById('btn-option-' + opt);
                if (opt === selectedOption) {
                    btn.style.backgroundColor = '#FFF5B8'; // Selection color
                    btn.style.borderColor = '#FFF5B8';
                } else {
