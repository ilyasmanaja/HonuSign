<x-student-layout>
    ...
    <div id="tutorial-overlay"
        ...
        <div class="bg-[#FFFEFA] brutal-border brutal-shadow p-8 md:p-12 rounded-[3rem] max-w-xl w-full flex flex-col items-center text-center relative transform scale-90 transition-transform duration-500"
            ...
            <div class="relative w-full max-w-sm h-64 bg-[#BEE9E8] brutal-border rounded-[2rem] p-4 mb-8 mx-auto overflow-hidden shadow-inner">
            ...
            <div class="flex flex-col gap-4 text-left w-full bg-[#F8FAFC] brutal-border p-6 rounded-2xl mb-8 shadow-sm">
                <div class="flex items-start gap-3">
                    ...
                    <p class="font-bold text-slate-700 text-sm md:text-base"><b>Seret Kartu</b>: Sentuh dan geser kartu perilaku di bawah ini.</p>
                </div>
                <div class="flex items-start gap-3">
                    ...
                    <p class="font-bold text-slate-700 text-sm md:text-base"><b>Kelompokkan</b>: Masukkan ke kotak hijau (Cinta Tanah Air) atau kotak merah (Tidak Cinta).</p>
                </div>
            </div>
    ...
    <div id="victory-modal"
        ...
        <div class="relative w-full max-w-[480px] aspect-square transform scale-90 transition-transform duration-500 select-none mx-4" id="victory-modal-content">
    ...
    <div id="success-toast" class="fixed inset-0 flex items-center justify-center z-[9000] pointer-events-none hidden transition-all duration-300 opacity-0 scale-90">
        <div class="bg-[#D4F1BE] brutal-border brutal-shadow p-8 rounded-[2.5rem] flex flex-col items-center gap-4 text-center max-w-sm w-full mx-4">
            <div class="w-20 h-20 bg-green-500 rounded-full flex items-center justify-center brutal-border brutal-shadow-sm">
    ...
    <div id="error-toast" class="fixed inset-0 flex items-center justify-center z-[9000] pointer-events-none hidden transition-all duration-300 opacity-0 scale-90">
        <div class="bg-[#FFB3B3] brutal-border brutal-shadow p-8 rounded-[2.5rem] flex flex-col items-center gap-4 text-center max-w-sm w-full mx-4">
            <div class="w-20 h-20 bg-red-500 rounded-full flex items-center justify-center brutal-border brutal-shadow-sm">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-12 h-12 text-black">
    ...
    <div class="max-w-5xl w-full px-6 py-12 flex flex-col items-center">
        ...
        <div class="w-full mb-10 max-w-3xl">
            ...
            <div class="w-full h-8 bg-white brutal-border brutal-shadow-sm rounded-2xl overflow-hidden p-1">
                <div class="h-full bg-[#FFD1E3] rounded-xl transition-all duration-1000 border-r-4 border-black" style="width: 83.3%"></div>
            </div>
        </div>

        <!-- Header Judul -->
        <div class="text-center mb-10">
            <h1 class="text-4xl md:text-5xl font-black text-black uppercase tracking-tighter transform -rotate-1 mb-2 flex justify-center items-center gap-3">
        ...
        <div class="w-full bg-[#FFD1E3] brutal-border brutal-shadow rounded-[3rem] p-6 md:p-8 mb-10">
            <div class="bg-[#FFFEFA] brutal-border brutal-shadow-sm rounded-[2rem] p-6 md:p-10">
                ...
                <div class="flex justify-center mb-8 border-b-4 border-slate-200 pb-6">
                    <h3 class="font-black text-black uppercase tracking-widest text-xl md:text-2xl text-center">Cinta &amp; Tidak Cinta Tanah Air</h3>
                </div>
                ...
                <div class="flex flex-col md:flex-row gap-8 justify-between w-full mb-10">
                    <!-- Positive Box -->
                    <div id="box-positif" class="brutal-border brutal-shadow rounded-[3rem] p-6 flex flex-col items-center justify-start min-h-[350px] w-full md:w-1/2 bg-[#D4F1BE] transition-transform duration-300">
                        <h4 class="font-black text-black uppercase tracking-wider text-lg md:text-xl text-center border-b-4 border-black pb-2 w-full mb-4">
                            Cinta Tanah Air
                        </h4>
                        <div id="list-positif" class="w-full flex flex-col gap-3 overflow-y-auto flex-grow max-h-[300px] p-2">
                            <!-- Correctly sorted cards will land here -->
                        </div>
                    </div>

                    <!-- Negative Box -->
                    <div id="box-negatif" class="brutal-border brutal-shadow rounded-[3rem] p-6 flex flex-col items-center justify-start min-h-[350px] w-full md:w-1/2 bg-[#FFB3B3] transition-transform duration-300">
                        <h4 class="font-black text-black uppercase tracking-wider text-lg md:text-xl text-center border-b-4 border-black pb-2 w-full mb-4">
                            Tidak Cinta Tanah Air
                        </h4>
                        <div id="list-negatif" class="w-full flex flex-col gap-3 overflow-y-auto flex-grow max-h-[300px] p-2">
                            <!-- Correctly sorted cards will land here -->
                        </div>
                    </div>
                </div>
                ...
                <div class="w-full flex flex-col items-center mt-6">
                    <p class="font-black text-black text-lg md:text-xl uppercase tracking-wider mb-4">Tarik Kartu di Bawah Ini:</p>
                    <div id="cards-pool" class="flex flex-wrap justify-center gap-6 max-w-4xl min-h-[14rem] items-center p-6 bg-slate-50 rounded-[2rem] border-4 border-dashed border-slate-200 w-full">
                        @php
                            // Ambil quiz tipe pilah_perilaku milik materi ini
                            $quiz = \App\Models\Quiz::where('materi_id', $materi->id)
                                ->where('tipe', 'pilah_perilaku')
                                ->first();

                            if ($quiz && $quiz->pilihan_data) {
                                $keberagaman = json_decode($quiz->pilihan_data, true);
                            } else {
                                // Fallback data statis awal
                                $keberagaman = [
                            ...
                            shuffle($keberagaman);
                            $jumlahKartu = count($keberagaman);
                        @endphp

                        @foreach ($keberagaman as $idx => $item)
                            <div id="card-{{ $idx }}"
                                data-positif="{{ filter_var($item['positif'], FILTER_VALIDATE_BOOLEAN) ? 'true' : 'false' }}" data-id="{{ $item['id'] }}"
                                ...
                                <div class="w-full h-28 bg-[#FFFEFA] border-b-4 border-black overflow-hidden relative pointer-events-none">
                                    <img src="{{ asset('images/tahap5/' . $item['gambar']) }}" alt="{{ $item['judul'] }}" class="w-full h-full object-cover pointer-events-none" onerror="this.style.display='none'">
                                </div>
                                <div class="p-3 flex items-center justify-center min-h-[3.5rem] pointer-events-none">
                                    <p class="font-black text-black text-xs text-center leading-snug pointer-events-none">{{ $item['judul'] }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
        ...
        <div class="w-full max-w-5xl flex justify-center gap-12 items-center mt-8 px-4">
            <div class="relative group/tooltip inline-block">
                <a href="{{ route('materi.index', ['mapel_slug' => $mapel->slug]) }}" onclick="tandaiSelesai(event, this.href, 5)"
            ...
            <div class="relative group/tooltip inline-block">
                <a href="{{ route('materi.belajar', ['mapel_slug' => $mapel->slug, 'step' => 6]) }}" onclick="tandaiSelesai(event, this.href, 5)"
    ...
    <style>
        ...
        @keyframes demo-drag {
            ...
            50% { transform: translate(-60px, -95px); }
        }
        @keyframes demo-hand {
            0%, 100% { transform: translate(10px, 40px); }
            50% { transform: translate(-70px, -50px); }
        }
        .animate-demo-drag { animation: demo-drag 3.5s ease-in-out infinite; }
        .animate-demo-hand { animation: demo-hand 3.5s ease-in-out infinite; }
    ...
    <script>
        ...
        // Memakai total jumlah kartu dinamis hasil hitung PHP seeder
        const totalCards = {{ $jumlahKartu }};
        ...
        function runTutorialAnimation() {
            ...
            if (!cardGreen || !cardRed || !cursor || !boxPositif || !boxNegatif) return;
        ...
        function dragMove(e) {
            ...
            activeDragCard.style.transform = `translate3d(${currentTranslateX}px, ${currentTranslateY}px, 0) rotate(${rotation}deg)`;
        ...
        function handleCorrectDrop(card, targetBox, x, y) {
            ...
            item.className = "bg-white p-3 rounded-2xl brutal-border flex items-center gap-3 w-full animate-bounce-short shadow-sm";
        ...
        function goToNextStage() {
            const nextUrl = "{{ route('materi.belajar', ['mapel_slug' => $mapel->slug, 'step' => 6]) }}";
